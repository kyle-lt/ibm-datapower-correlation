...
<xsl:stylesheet version="1.0" 
                ...
                extension-element-prefixes="dp dpgui"
  ...
  <xsl:template mode="request" match="operation">
    ...
    <xsl:variable name="httpHeaders">
        ...
        <!-- extract a log correlation ID for the session, if it exists, and set it for the call -->
        <xsl:variable name="logid">
            <xsl:if test="function-available('dpgui:get-user-log-correlation')">
                <xsl:value-of select="dpgui:get-user-log-correlation($session)/logcorrelation"/>
            </xsl:if>
        </xsl:variable>
        <xsl:if test="not($logid='')">
            <header name="X-Global-Transaction-ID"><xsl:value-of select="$logid" /></header>
        </xsl:if>
    </xsl:variable>
