...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="ENFORCE-FACTORING" select="'false'" />

    <xsl:param name="PLATFORM" select="'unknown'" />
    <xsl:param name="PRODUCT" select="'unknown'" />
    <xsl:param name="EDITION" select="'unknown'" />
    <xsl:param name="IS-TENANT" select="'false'" />
   ...
   <!-- default to false for use by dpe:transform calls to clixform
    ...
    <xsl:template mode="cli-object" match="StylePolicy">
        <xsl:if test="not(@read-only='true') or $delta-config">
  ...
  <!-- ************************************************************ -->
    ...
    <xsl:template mode="cli-object" match="StylePolicyAction">
        ...
        <xsl:if test="(not($IsLocal/local) and (not(@read-only='true') or $delta-config))">
    ...
    <!-- ************************************************************ -->
    <!-- SSH Server Profile -->
    <!-- Note: SSH Server Profile can not be deleted. -->
    <!-- ************************************************************ -->    
    <xsl:template mode="cli-object" match="SSHServerProfile">
      <xsl:call-template name="CryptoWrapper">
        <xsl:with-param name="cmdline">
          <xsl:apply-templates mode="CanonicalObject" select="."/>
        </xsl:with-param>
    ...
    <!-- Note: SSH Domain Client Profiles can not be deleted. -->
    <!-- ************************************************************ -->    
    <xsl:template mode="cli-object" match="SSHDomainClientProfile">
      <xsl:call-template name="CryptoWrapper">
        <xsl:with-param name="cmdline">
          <xsl:apply-templates mode="CanonicalObject" select="."/>
        </xsl:with-param>
   ...
   <xsl:template mode="HTTPUserAgent" match="*">
    ...
    <xsl:template mode="cli-object" match="WebGUI">
        <xsl:value-of select="concat($eol, '%if% available ', dpfunc:quoesc('save-config overwrite'), $eol)"/>
        <xsl:choose>
    ...
    <xsl:template mode="DNSNameService" match="StaticHosts">
        <xsl:value-of select="concat('  static-host ', dpfunc:quoesc(Hostname), ' ', dpfunc:quoescne(IPAddress), ' ', dpfunc:quoesc(UserSummary), $eol)"/>
    </xsl:template>
    ...
    <xsl:template mode="cli-object" priority="-100" match="*[$config-objects-index/self::object/@name=local-name()]">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:variable name="isFirstOfClass" select="../*[$type = local-name()][1]/@name = @name"/>
                <xsl:variable name="object-schema" select="$schema/config-objects//object[@name=$type]"/>
                <xsl:choose>
                    <xsl:when test="$ENFORCE-FACTORING='true' and $object-schema/@factored-config='true'">
                        <xsl:choose>
                            <xsl:when test="$isFirstOfClass='true'">
                                <xsl:variable name="exec" select="concat($eol, 'exec config:///auto-', $object-schema/cli-alias, '.cfg' ,$eol)"/>
    ...
    <xsl:template mode="CanonicalObject" match="*">
        ...
        <xsl:if test="(not($pNode/@persisted='false') and not(@persisted='false')) or $delta-config">
  ...
  <func:function name="dpfunc:propIsPersisted">
      <xsl:param name="pNode" />
      <xsl:param name="currentPlatform" />
      <xsl:param name="isTenant" />

      <xsl:variable name="is-persisted">
          <xsl:variable name="as-string">
              <xsl:choose>
                  <xsl:when test="$pNode/platforms-attr[platform=$currentPlatform]">
                      <xsl:value-of select="$pNode/platforms-attr/@persisted"/>
                  </xsl:when>
                  <xsl:when test="$isTenant='true' and $pNode/platforms-attr[platform='tenant']">
                      <xsl:value-of select="$pNode/platforms-attr/@persisted"/>
                  </xsl:when>
                  <xsl:otherwise>
                      <xsl:value-of select="$pNode/@persisted"/>
                  </xsl:otherwise>
              </xsl:choose>
          </xsl:variable>
          <xsl:choose>
              <xsl:when test="$as-string='false'">
      ...
      <func:result select="$is-persisted"/>
  </func:function>
  ...
  <xsl:template mode="DefaultProperty" match="*">
    ...
    <xsl:choose>
        ...
        <xsl:when test="($pNode/@suppress='true')"/>

        <!-- skip unpersisted properties in save config mode -->
        <xsl:when test="dpfunc:propIsPersisted($pNode, $PLATFORM, $IS-TENANT)='false'
  ...
  <xsl:template match="args[action='FlushDocumentCache']" mode="cli-actions">
    <xsl:choose>
      <xsl:when test="string(MatchKey)">
          <xsl:value-of select="concat(
                                 'configure terminal', $eol,
                                 'documentcache ', dpfunc:quoesc(XMLManager), $eol,
                                 'clear ', dpfunc:quoesc(MatchPattern), ' ', dpfunc:quoesc(ExpireEntries), ' ', dpfunc:quoesc(MatchKey), $eol,
                                 'cancel', $eol)"/>
      </xsl:when>
      <xsl:when test="string(ExpireEntries)">
          <xsl:value-of select="concat(
                                 'configure terminal', $eol,
                                 'documentcache ', dpfunc:quoesc(XMLManager), $eol,
                                 'clear ', dpfunc:quoesc(MatchPattern), dpfunc:quoesc(ExpireEntries), $eol,
                                 'cancel', $eol)"/>
      </xsl:when>
  ...
  <xsl:template match="args[action='UniversalPacketCaptureDebug']" mode="cli-actions">
      ...
      <xsl:value-of select="concat(' ', dpfunc:quoesc(MaxSize), ' ', dpfunc:quoesc(MaxPacketSize), ' ', 
                                   dpfunc:quoesc(Filter), ' ', dpfunc:quoesc(SSLKeyLog), $eol)"/>
  </xsl:template>
...
<!-- action-objects/TenantFirmware.xml -->
 <xsl:template match="args[action='TenantInstall']" mode="cli-actions">
     <xsl:message dpe:priority="error">clixform TenantInstall: <xsl:copy-of select="."/></xsl:message> 
      <!-- this command expects a bare filename here -->
      <xsl:variable name="bare-file">
          <xsl:choose>
              <xsl:when test="starts-with(File, 'image:///')">
      ...
      <xsl:variable name="accept-license">
          <xsl:choose>
              <xsl:when test="AcceptLicense='on'">
                   <xsl:text>accept-license </xsl:text>
              </xsl:when>
              <xsl:otherwise>
                   <xsl:text> </xsl:text>
      ...
      <xsl:value-of select="concat(
                            'configure terminal', $eol,
                            'flash', $eol,
                            'tenant-install ', $accept-license, $name, ' ', dpfunc:quoesc($bare-file), $eol,
                            'exit', $eol, 'exit', $eol)"/>
  </xsl:template>
